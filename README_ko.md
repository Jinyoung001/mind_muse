...
MIND MUSE는 AI 외계인 캐릭터를 통해 **파인만 기법(Feynman Technique)**을 구현한 Flutter 앱입니다. 학생이 어떤 이미지든 업로드하고 — 교과서 페이지, 다이어그램, 제품 라벨, 주변 사물 사진 등 — 관심 영역에 직접 드로잉을 표시하면, 외계인 조사관이 모든 것을 완전히 엉뚱하게 해석합니다. 외계인을 바로잡으려면 학생이 자신의 이해를 직접 말로 표현해야 합니다 — 그 과정에서 진짜 학습이 일어납니다.
...
```
📚 어떤 이미지든 업로드 (교과서, 다이어그램, 실물 사진 — 무엇이든)
        ↓
✏️  핵심 영역에 원/밑줄 드로잉
        ↓
🚀 "탐사 질문하기" 버튼 탭
...
allContents.add(Content('user', [
  DataPart('image/png', imageBytes),   // 업로드 이미지 + 드로잉 스트로크 합성 PNG
  TextPart(message),
]));
...
1. **홈 화면**: "탐사 시작" 버튼 탭
2. **이미지 선택**: 갤러리 또는 카메라에서 아무 사진이나 선택 — 교과서, 다이어그램, 실물 사물, 이해하고 싶은 무엇이든
...
**어떤 주제든, 어떤 모델이든.** 학습 메커니즘이 AI가 아닌 학생의 머릿속에 있으므로, 사진을 찍을 수 있는 어떤 대상에도 적용됩니다. 생물 다이어그램, 화학 방정식, 역사 지도, 제품 라벨, 주변의 모든 사물 — 원을 그릴 수 있다면 외계인이 오해할 수 있습니다. 모델 크기도 마찬가지입니다: AI가 학습을 이끌기 위해 뛰어날 필요는 없습니다 — 일관되고 흥미롭게 *틀리기만* 하면 됩니다. 인지적 노동은 가르침을 예상하는 태도와 설명 행위 모두에서 나옵니다. AI는 그저 혼란스러운 청자일 뿐입니다.

교육학 연구가 이를 뒷받침합니다: *프로테제 효과(Protégé Effect)* — 누군가에게 가르칠 것을 예상할 때 나타나는 측정 가능한 학습 향상 — 에 따르면, 학습을 가르침으로 설정하면 수동적 학습보다 더 깊은 처리, 더 강한 기억 부호화, 더 나은 장기 기억을 활성화합니다. Nestojko et al. (2014)는 결정적인 뉘앙스를 밝혔습니다: 이 효과는 학습 **전에** 가르칠 것을 예상하는 것에서 비롯됩니다 — 학습 전에 가르칠 것이라는 기대를 가진 참가자들이 더 많이 기억하고 더 잘 조직화된 지식을 보였습니다. MIND MUSE는 이 기대를 첫 탭부터 설계에 내장합니다: 학생이 이미지에 드로잉하는 순간, 외계인이 오해할 것이고 설명해야 한다는 걸 이미 압니다. 대화가 시작되기 전에 '가르치는 태도'가 먼저 활성화됩니다.

이 효과는 주로 학생의 가르치는 행위 자체에서 나옵니다 — *Betty's Brain* (Leelawong & Biswas, 2008) 연구에서 학생들이 가상의 AI 에이전트에게 가르쳤을 때 유의미한 학습 향상이 나타났으며, 핵심 메커니즘이 청자의 능력보다 학생의 가르치는 행위에 있음을 시사했습니다. AI 기반 파인만 봇에 관한 파일럿 연구(arXiv 2506.09055, 2025)에서도 'AI에게 가르치기' 방식이 수동적 학습 그룹보다 학습 성과와 자신감 모두에서 높은 결과를 보였습니다.

MIND MUSE는 이 교육학적 루프를 자동화하고 접근 가능하게 만듭니다 — 스마트폰이 있는 학생이라면 어떤 주제든, 튜터나 스터디 파트너 없이도 몇 분 만에 파인만 기법을 연습할 수 있습니다.
...
*어떤 이미지든 대화가 됩니다. 혼란스러운 외계인 한 명이 곧 학습 기회입니다.*
