...
Gemma 4의 체인-오브-소트가 최종 답변 전에 추론 아티팩트(초안 문장, 괄호 안 영어 번역, 내부 해설 등)를 노출하는 경우가 있습니다. 커스텀 `cleanResponse()` 파이프라인이 이를 처리합니다:
...
6. 근사 중복 문장 제거 (60% 이상 문자 겹침 임계값 — Gemma의 가끔씩 나오는 문장 반복 처리)
